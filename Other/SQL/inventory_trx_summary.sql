...
FROM (
    ...
    FROM iminvtrx_sql as [iminvtrx_sql] WITH (NOLOCK)
    left join LEAHYCONSULTING.#OPT_InventoryStatus as [LEAHYCONSULTING.#OPT_InventoryStatus] WITH (NOLOCK)
    ...
    left join poordhdr_sql as [poordhdr_sql] WITH (NOLOCK)
        on iminvtrx_sql.doc_ord_no = [poordhdr_sql].ord_no
       and iminvtrx_sql.source = 'R'
    left join apvenfil_sql as [apvenfil_sql] WITH (NOLOCK)
        on [apvenfil_sql].vend_no = [poordhdr_sql].vend_no
    left join oeordhdr_sql as [oeordhdr_sql] WITH (NOLOCK)
        on iminvtrx_sql.doc_ord_no = [oeordhdr_sql].ord_no
       and iminvtrx_sql.source = 'O'
    left join arcusfil_sql as [arcusfil_sql] WITH (NOLOCK)
